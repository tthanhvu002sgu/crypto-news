...
- **Scanner Shortlist Engine v8 (Lọc Sức Mạnh Trước, Xác Nhận Setup Sau):** Tái cấu trúc triệt để cơ chế xếp hạng theo tổng điểm cũ thành pipeline 5 cửa lọc bắt buộc: `Dữ liệu hợp lệ → Đủ thanh khoản → Đạt sức mạnh → Có setup → Kiểm tra vị trí hiện tại`. Phân tích đa khung thời gian: 1D bối cảnh, 4H xu hướng, 1H xác nhận setup; hỗ trợ 2 chiều LONG/SHORT với 2 mẫu hình kỹ thuật: **Pullback tiếp diễn** (Continuation Pullback) và **Breakout–Retest**. Phân biệt rõ ràng 5 trạng thái (`READY`, `FORMING`, `WATCH`, `EXTENDED`, `DATA_INCOMPLETE`). Đo lường khoảng cách đến vùng kích hoạt và khoảng đệm vô hiệu hóa bằng ATR; tách biệt hoàn toàn 2 metric **Volume co lại** và **Biên độ co hẹp**; hiển thị mức tham chiếu (Trigger Zone, Stop Level, Next Resistance) và R:R trước/sau chi phí; bảo tồn quota momentum (30 liquid + 10 gainer + 10 loser). Setup đã vô hiệu được lưu bằng cờ bền vững và khôi phục trước lần quét mới; cập nhật outcome không ghi đè trạng thái mới nhất.
...
- **Lưu trữ cục bộ & Persistence Multi-layer:**
  ...
  - **IndexedDB Research Storage:**
    ...
    - `CryptoScannerResearch` (store `scanner_events`): giữ snapshot READY, các trường latest và cờ `isInvalidated` riêng biệt; khôi phục khóa vô hiệu từ cờ hoặc trạng thái legacy. Lưu các setup có formedAt ngoài Top 5; cập nhật outcome bằng transaction riêng sau khi đọc lại bản ghi mới nhất. Outcome dùng nến cùng thị trường, từ cặp confirmationPrice/confirmedAt khi có đủ dữ liệu, nếu thiếu giá xác nhận dùng giá ban đầu/mốc phát hiện được cố định. Kết quả theo mốc xác nhận lịch sử không đại diện cho giao dịch thực thi lúc scanner phát hiện muộn.
...
- `src/services/scannerEventStore.js` — IndexedDB `CryptoScannerResearch`: dedupe theo setup, giữ snapshot READY và cờ vô hiệu; `buildHydratedSetupRegistry` phục hồi khóa sau reload; `saveScannerEventOutcome` chỉ cập nhật outcome và thời điểm đánh giá, không ghi đè giá/trạng thái/mức cấu trúc mới nhất. Đo return 4H/24H, MFE/MAE và resolution từ OHLC.
...
- `src/services/biasEngine.js` — Tính toán điểm xu hướng BTC (-100 đến +100) theo 4 pillars với Spot & Futures CVD từ Binance Benchmark Proxy trong 3-layer regime.
...
### [2026-09-19] Sửa vòng đời Scanner: scan, reload và outcome `(FIX)`
- **Thay đổi:** Bổ sung import bị thiếu của `queryScannerEvents`; khôi phục invalidation bằng cờ bền vững, hỗ trợ `latestStatus` legacy; lưu setup ngoài Top 5. Ưu tiên khóa đã lưu trước trạng thái bộ nhớ.
- **Dữ liệu:** Tách transaction cập nhật outcome khỏi upsert scan; đợi commit khi lưu batch. Giữ `confirmationPrice = null` nếu thiếu; cố định mốc phát hiện tại lần READY đầu tiên để refresh không làm trôi cửa sổ đo.
- **Regression:** Thêm test `runFullScan` với API giả lập; lưu–reload setup vô hiệu; outcome fetch chồng refresh; thiếu giá xác nhận và giữ stop/target cũ.
- **Verify:** 47/47 test scanner pass; toàn bộ `npm test` pass; `npm run build` pass (còn cảnh báo bundle >500kB đã có trong backlog); ESLint cho 5 file JS thay đổi pass; `git diff --check` pass.
- **Giới hạn:** Chưa smoke-test browser/API live. Không tự dựng lại cờ vô hiệu hoặc giá xác nhận đã bị bản cũ ghi đè mất; dữ liệu lịch sử đó cần được xem là chưa kiểm chứng.
...
- **Thay đổi chính:**
  - *Dọn dẹp service:* Xóa thư mục `src/services/orderFlow/` (gồm 7 adapter WebSocket và IndexedDB store đa sàn); tinh giản `package.json` test script `test:orderflow`.
  - *Giao diện HftRadarTab:* Loại bỏ nút chuyển đổi `MULTI / BINANCE`, health strip 7 stream, bảng venue contribution và feed divergence đa sàn. Đưa giao diện CVD & Order Flow về giao diện Binance sạch sẽ với đầy đủ badges proxy, dual Y-axis, Flow Pressure Cards và Volume Ratio gauges.
  - *Market Bias Engine:* Tinh giản `src/services/biasEngine.js` để tính Spot & Futures CVD trực tiếp từ chuỗi dữ liệu nến đóng chuẩn của Binance.
- **Files / areas chạm:** `src/App.jsx`, `src/components/HftRadarTab.jsx`, `src/services/biasEngine.js`, `src/services/biasEngine.test.js`, `src/services/orderFlow/*` (đã xóa), `package.json`, `README.md`.
- **Ảnh hưởng README:** §1 / §2 / §3 / §4
- **Verify:** `npm test` pass toàn bộ (74/74 unit tests); `npm run build` pass không lỗi; `git diff --check` pass.
...
- **Mode / Type / Action / Lane:** FEATURE / BUGFIX / EXECUTE / FAST
- **Tóm tắt:** Loại bỏ hai autoscale Y độc lập khiến cùng một vị trí trên chart mang hai giá trị khác nhau (ví dụ Futures `−$400M` nhưng đọc chéo sang trục Spot thành vùng dương, hoặc Spot triệu USD bị đối chiếu nhầm với tick tỷ USD).
- **Thay đổi chính:** Gắn cả hai dataset vào scale `y` dùng chung, chỉ giữ một zero baseline và một hệ tick `fmtCvdUsd`; thêm nhãn `CVD RÒNG · SPOT + FUTURES` để chỉ rõ đây là scale tiền tệ chung.
- **Files / areas chạm:** `src/components/HftRadarTab.jsx`, `README.md`
- **Ảnh hưởng README:** §1 / §3
- **Verify:** Production build, CVD regression tests và smoke-test trực quan 1H/7D/30D; cùng một đường ngang chỉ còn một trị số Y cho cả Spot và Futures.
- **Notes / nợ kỹ thuật:** Không thêm unit test UI vì repo chưa có harness render Chart.js; dùng browser smoke-test làm regression evidence.
